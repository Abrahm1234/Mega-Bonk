...
@export var irregular_bit_source_mesh: Mesh
# (Deprecated) irregular_bit_height removed; floor thickness uses floor_thickness like voxel grid.
@export var irregular_bit_y_offset: float = 0.0
@export var irregular_bit_inset: float = 0.0 # 0=full cell, 0.1=shrink 10% toward center
...
@export_category("Irregular Fill (Noise)")
@export var irregular_use_noise_fill: bool = true
@export_range(0.0001, 10.0, 0.0001) var irregular_noise_frequency: float = 0.15
@export var irregular_noise_offset: Vector2 = Vector2.ZERO
@export_range(0.0001, 10.0, 0.0001) var irregular_noise_coord_scale: float = 1.0
@export_range(-1.0, 1.0, 0.01) var irregular_noise_bias: float = 0.0
@export var irregular_noise_invert: bool = false
...
@export var city_empty_as_park: bool = true
...
@onready var dual_points_mmi: MultiMeshInstance3D = _find_mmi("ArenaDualGridPoints")
@onready var dual_bits_filled_mmi: MultiMeshInstance3D = _find_mmi("ArenaDualBits_Filled")
@onready var dual_bits_empty_mmi: MultiMeshInstance3D = _find_mmi("ArenaDualBits_Empty")
@onready var irregular_bits_filled_mi: MeshInstance3D = get_node_or_null("Arena/IrregularBitsFilled") as MeshInstance3D
@onready var irregular_bits_empty_mi: MeshInstance3D = get_node_or_null("Arena/IrregularBitsEmpty") as MeshInstance3D
@onready var irregular_walls_mi: MeshInstance3D = get_node_or_null("Arena/IrregularWalls") as MeshInstance3D
...
var _ir_face_bits: PackedByteArray = PackedByteArray()  # per-quad occupancy for irregular mode
var _ir_face_border_dist: PackedInt32Array = PackedInt32Array()
var _ir_bounds_center2: Vector2 = Vector2.ZERO
var _ir_bounds_radius: float = 1.0
...
func _find_mmi(node_name: String) -> MultiMeshInstance3D:
	# Supports both layouts:
	#  - ArenaAutoGen/ArenaDualBits_Filled
	#  - ArenaAutoGen/Arena/ArenaDualBits_Filled
	var n: Node = get_node_or_null(node_name)
	if n == null:
		n = get_node_or_null("Arena/" + node_name)
...
func _ready() -> void:
	...
	# Ensure the solid irregular debug floor is always hidden.
...
func _build_ir_face_neighbors(face_count: int, dual_edges: Array[Vector2i]) -> Array[PackedInt32Array]:
	# Build adjacency list for faces using dual_edges (undirected).
	var tmp: Array = []
	tmp.resize(face_count)
	for i in range(face_count):
		tmp[i] = {}
	for e in dual_edges:
		var a: int = e.x
		var b: int = e.y
		if a < 0 or b < 0 or a >= face_count or b >= face_count:
			continue
		(tmp[a] as Dictionary)[b] = true
		(tmp[b] as Dictionary)[a] = true
	var out: Array[PackedInt32Array] = []
	out.resize(face_count)
	for i in range(face_count):
		out[i] = PackedInt32Array((tmp[i] as Dictionary).keys())
	return out


func _ir_face_is_outer(face_i: int) -> bool:
	# A face is considered "outer" if any of its vertices is a boundary vertex.
	if face_i < 0 or face_i >= _ir_quads.size():
		return true
	var q: PackedInt32Array = _ir_quads[face_i]
	for vid in q:
		var v: int = int(vid)
		if v >= 0 and v < _ir_boundary.size() and _ir_boundary[v] != 0:
			return true
	return false
...
func _build_ir_face_bits(neighbors: Array[PackedInt32Array]) -> PackedByteArray:
	...
	match irregular_layout_mode:
		...
		_:
			_fill_ir_bits_noise(bits)

	# --- 2) Border clearance ---
	if border > 0 and _ir_face_border_dist.size() == face_count:
		for i in range(face_count):
...
func _fill_ir_bits_noise(bits: PackedByteArray) -> void:
	...
	for i in range(face_count):
		...
		var nx: float = (p.x + irregular_noise_offset.x) * irregular_noise_coord_scale
		var ny: float = (p.y + irregular_noise_offset.y) * irregular_noise_coord_scale
		var v: float = noise.get_noise_2d(nx, ny)
		if irregular_noise_invert:
			v = -v
...
func _compute_ir_face_border_distances(neighbors: Array[PackedInt32Array]) -> PackedInt32Array:
	...
	dist.resize(face_count)
	for i in range(face_count):
		dist[i] = 0x3fffffff

	var queue: Array[int] = []
	for i in range(face_count):
		if _ir_face_is_outer(i):
			dist[i] = 0
			queue.append(i)
...
func _keep_largest_connected_faces(bits: PackedByteArray, neighbors: Array[PackedInt32Array]) -> PackedByteArray:
	var n: int = bits.size()
	var seen: PackedByteArray = PackedByteArray()
	seen.resize(n)
	for i in range(n):
		seen[i] = 0
	var best_component: PackedInt32Array = PackedInt32Array()
	for i in range(n):
		if bits[i] == 0 or seen[i] != 0:
			continue
		var q: Array[int] = [i]
		seen[i] = 1
		var comp: Array[int] = []
		while not q.is_empty():
			var a: int = q.pop_back()
			comp.append(a)
			for b in neighbors[a]:
				var bi: int = int(b)
				if bi < 0 or bi >= n:
					continue
				if bits[bi] == 0 or seen[bi] != 0:
					continue
				seen[bi] = 1
				q.append(bi)
		if comp.size() > best_component.size():
			best_component = PackedInt32Array(comp)
	# Keep only faces in best_component
...
func _quad_inradius(points2: Array[Vector2], q: PackedInt32Array, c: Vector2) -> float:
	# Minimum distance from center to any edge (in 2D).
	var r: float = INF
	for e in range(4):
		var a: Vector2 = points2[int(q[e])]
		var b: Vector2 = points2[int(q[(e + 1) % 4])]
		var ab: Vector2 = b - a
		var t: float = 0.0
		var denom: float = ab.length_squared()
		if denom > 1e-8:
			t = clamp(((c - a).dot(ab)) / denom, 0.0, 1.0)
		var p: Vector2 = a + ab * t
		r = min(r, c.distance_to(p))
	if r == INF:
		return 0.0
	return r


func _ensure_irregular_bits_nodes() -> void:
	if irregular_bits_filled_mi == null:
		var arena_node := get_node_or_null("Arena") as Node3D
		if arena_node != null:
			irregular_bits_filled_mi = MeshInstance3D.new()
			irregular_bits_filled_mi.name = "IrregularBitsFilled"
			arena_node.add_child(irregular_bits_filled_mi)
	if irregular_build_empty_bits_mesh and irregular_bits_empty_mi == null:
		var arena_node2 := get_node_or_null("Arena") as Node3D
		if arena_node2 != null:
			irregular_bits_empty_mi = MeshInstance3D.new()
			irregular_bits_empty_mi.name = "IrregularBitsEmpty"
			arena_node2.add_child(irregular_bits_empty_mi)


func _ensure_irregular_walls_node() -> void:
	if irregular_walls_mi == null:
		var arena_node := get_node_or_null("Arena") as Node3D
		if arena_node != null:
			irregular_walls_mi = MeshInstance3D.new()
			irregular_walls_mi.name = "IrregularWalls"
			arena_node.add_child(irregular_walls_mi)



enum IrTileKind { FULL, EDGE, CORNER, INVERSE_CORNER, CHECKER }

func _mesh_primary_material(m: Mesh) -> Material:
	if m == null:
		return null
	if m.get_surface_count() <= 0:
		return null
	return m.surface_get_material(0)

func _pick_irregular_bit_source_mesh() -> Mesh:
	# Used as fallback when variant arrays are empty.
	if irregular_bit_source_mesh != null:
		return irregular_bit_source_mesh
	if floor_mesh_full_variants.size() > 0 and floor_mesh_full_variants[0] != null:
		return floor_mesh_full_variants[0]
	if mesh_full != null:
		return mesh_full
	if floor_mesh_edge_variants.size() > 0 and floor_mesh_edge_variants[0] != null:
		return floor_mesh_edge_variants[0]
	if mesh_edge != null:
		return mesh_edge
	return null
...
func _build_face_edge_neighbors(quads: Array[PackedInt32Array]) -> Array[PackedInt32Array]:
	...
	neighbors.resize(quads.size())
	for i in range(quads.size()):
		neighbors[i] = PackedInt32Array([-1, -1, -1, -1])

	var edge_map := {} # key -> [face, edge]
	for fi in range(quads.size()):
		...
		for ei in range(4):
			var a: int = int(q[ei])
			var b: int = int(q[(ei + 1) % 4])
			var lo: int = min(a, b)
			var hi: int = max(a, b)
			var key: String = str(lo) + ":" + str(hi)

			if edge_map.has(key):
				var other = edge_map[key]
				var ofi: int = int(other[0])
				var oei: int = int(other[1])
				neighbors[fi][ei] = ofi
				neighbors[ofi][oei] = fi
			else:
				edge_map[key] = [fi, ei]
	return neighbors

func _ir_face_edge_mask(face_i: int) -> int:
	# bit=1 if this edge borders a different occupancy or outside
	if face_i < 0 or face_i >= _ir_quads.size():
		return 0b1111
	if _ir_face_bits.size() != _ir_quads.size():
		return 0b1111

	var self_filled: int = int(_ir_face_bits[face_i] != 0)
	var mask: int = 0
	for ei in range(4):
		var n: int = -1
		if face_i < _ir_face_edge_neighbors.size():
			n = int(_ir_face_edge_neighbors[face_i][ei])
		var open: bool = false
		if n == -1:
			open = true
		else:
			var other_filled: int = int(_ir_face_bits[n] != 0)
			open = (other_filled != self_filled)
		if open:
			mask |= (1 << ei)
	return mask
...
func _classify_mask(mask: int) -> Dictionary:
	...
	if bits == 1:
		var ei: int = _first_set_bit(mask)
		return {"kind": IrTileKind.EDGE, "rot": ei}

	if bits == 3:
		var closed: int = ((~mask) & 0b1111)
		var ei_closed: int = _first_set_bit(closed)
		return {"kind": IrTileKind.INVERSE_CORNER, "rot": ei_closed}

	if bits == 2:
		for i in range(4):
			var adj: int = (1 << i) | (1 << ((i + 1) % 4))
			if mask == adj:
				return {"kind": IrTileKind.CORNER, "rot": i}
		# opposite edges
		return {"kind": IrTileKind.CHECKER, "rot": 0}

	# bits == 4, treat as checker/full
	return {"kind": IrTileKind.FULL, "rot": 0}
...
func _append_deformed_variant(
	...
	if not sts.has(key):
		...
		var mat: Material = _mesh_primary_material(src)
		if mat != null:
			st.set_material(mat)
		entry = {"st": st, "src": src}
		sts[key] = entry
		...
		entry = sts[key]
	var st2: SurfaceTool = entry["st"]
	IrregularMeshDeformer.append_deformed_mesh_to_surfacetool(st2, src, quad_rot, up, height)


func _commit_multisurface(sts: Dictionary) -> ArrayMesh:
	if sts.is_empty():
		return null
	var out := ArrayMesh.new()
	# Keep deterministic ordering for reproducibility
	var keys: Array = sts.keys()
	keys.sort()
	for k in keys:
		var st: SurfaceTool = sts[k]["st"]
		st.generate_normals()
		var tmp: ArrayMesh = st.commit()
		if tmp == null or tmp.get_surface_count() == 0:
			continue
		var arrays = tmp.surface_get_arrays(0)
		out.add_surface_from_arrays(Mesh.PRIMITIVE_TRIANGLES, arrays)
		var sidx := out.get_surface_count() - 1
		var mat := tmp.surface_get_material(0)
		if mat != null:
			out.surface_set_material(sidx, mat)
	return out


func _build_irregular_deformed_floor_mesh(want_filled: bool) -> ArrayMesh:
	if _ir_quads.is_empty():
		return null
	if _ir_face_bits.size() != _ir_quads.size():
		return null
	# Ensure edge-neighbor map exists.
	if _ir_face_edge_neighbors.size() != _ir_quads.size():
		_ir_face_edge_neighbors = _build_face_edge_neighbors(_ir_quads)
	...
	for fi in range(_ir_quads.size()):
		...
		var p0: Vector3 = gx * Vector3(p0_2.x, 0.0, p0_2.y) + up * (irregular_bit_y_offset - floor_thickness)
		var p1: Vector3 = gx * Vector3(p1_2.x, 0.0, p1_2.y) + up * (irregular_bit_y_offset - floor_thickness)
		var p2: Vector3 = gx * Vector3(p2_2.x, 0.0, p2_2.y) + up * (irregular_bit_y_offset - floor_thickness)
		var p3: Vector3 = gx * Vector3(p3_2.x, 0.0, p3_2.y) + up * (irregular_bit_y_offset - floor_thickness)

		# Center-anchored inset so deformation expands from the cell center (voxel-like).
...
func _build_irregular_deformed_walls_mesh() -> ArrayMesh:
	if _ir_quads.is_empty():
		return null
	if _ir_face_bits.size() != _ir_quads.size():
		return null
...
func _update_irregular_deformed_bits() -> void:
	# Hide legacy multimesh debug boxes in irregular mode
	...
	if _ir_face_bits.size() != _ir_face_centers2.size():
		_ir_face_neighbors = _build_ir_face_neighbors(_ir_face_centers2.size(), _ir_dual_edges)
...
func _generate_irregular_patch_grid() -> void:
	...
	var tr: int = max(patch_tile_radius, 0)

	var res: Dictionary
	if tr <= 0:
		res = IrregularQuadGrid.build_hex_patch(
			rings,
			cell_size,
			dissolve_chance,
			subdivide_quads,
		...
		res = IrregularQuadGrid.build_tiled_hex_grid(
			rings,
			cell_size,
			tr,
			dissolve_chance,
...
func _update_irregular_wire_debug() -> void:
	...
	_update_irregular_dual_grid_points()

	_update_irregular_deformed_bits()



func _update_irregular_dual_grid_points() -> void:
	if dual_points_mmi == null:
		return
	...
	mm.transform_format = MultiMesh.TRANSFORM_3D
	mm.mesh = sphere
	mm.instance_count = _ir_face_centers2.size()

	for i in range(_ir_face_centers2.size()):
		var c: Vector2 = _ir_face_centers2[i]
		var p: Vector3 = gx * Vector3(c.x, 0.0, c.y) + up * wire_grid_y
		mm.set_instance_transform(i, Transform3D(Basis.IDENTITY, p))
...
func _hide_irregular_floor_mesh() -> void:
	if irregular_floor_mi == null:
		return
	irregular_floor_mi.visible = false
	irregular_floor_mi.mesh = null
...
func _city_variant_from_base(base_id: String) -> String:
	match base_id:
...
func _build_floor_dual_variant_multimeshes() -> void:
	...
	for y in range(grid_h):
		for x in range(grid_w):
			...
			var yaw: float = 0.0
			if has_city_wfc:
				variant_id = _city_variant_from_base(_city_base_at(x, y))
