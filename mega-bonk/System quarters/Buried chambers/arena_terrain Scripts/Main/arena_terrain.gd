...
@export var overhang_enable: bool = true
@export_range(0.0, 8.0, 0.01) var overhang_min_drop: float = 0.75
...
func _ensure_debug_tag_material() -> ShaderMaterial:
	...
	sm.shader = load("res://System quarters/Buried chambers/arena_terrain Shaders/debug_tag_colors.gdshader")
...
func _ready() -> void:
	...
	if use_rock_shader:
		var sm := ShaderMaterial.new()
		sm.shader = load("res://System quarters/Buried chambers/arena_terrain Shaders/blocky_rock.gdshader")
...
func _build_surface_registry_from_layout(n: int) -> void:
	...
	for z in range(n):
		for x in range(n):
			...
			if is_ramp:
				_register_surface(SurfaceKind.RAMP, Vector2i(x, z), SurfaceSide.NONE, Vector2i(-1, -1), SurfaceSide.NONE, verts, Vector3.ZERO, {"floor_level": floor_level, "low_level": low_level, "high_level": high_level, "ramp_dir": ramp_dir, "floor_mask": floor_mask, "floor_rot": floor_rot, "module_key": _surface_module_key(SurfaceKind.RAMP, x, z, {"ramp_dir": ramp_dir, "low_level": low_level, "high_level": high_level}, n)})
			else:
